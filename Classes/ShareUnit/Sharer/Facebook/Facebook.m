...
@synthesize accessToken = _accessToken,expirationDate = _expirationDate;
...
- (id)initWithAppId:(NSString *)app_id {
  ...
  if (self) {
	  ...
	  _accessToken = [[[NSUserDefaults standardUserDefaults] objectForKey:FACEBOOK_STORED_TOKEN_NAME] copy];
	  [_expirationDate release];
	  _expirationDate = [[NSUserDefaults standardUserDefaults] objectForKey:FACEBOOK_EXPIRATION_DATE];
  }
  return self;
}
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
	...
	loginDialog_ = [[LoginDialog alloc] initWithURLStr:loginDialogURL params:params delegate:self];
	[loginDialog_ setNeedWebView:YES];
	[loginDialog_ setConnectTitle:@"Connect to Facebook"];
	[loginDialog_ setIconImg:[UIImage imageNamed:@"LoginDialog.bundle/icons/facebook.png"]];
	[loginDialog_ show];
...
- (FBRequest*)postVideoWithMethodName:(NSString *)methodName
					  ...
					  andHttpMethod:(NSString *)httpMethod
						andDelegate:(id <FBRequestDelegate>)delegate {
	NSString * fullURL = @"https://api-video.facebook.com/restserver.php?method=facebook.video.upload";
	return [self openUrl:fullURL
				  params:params
			  httpMethod:httpMethod
				delegate:delegate];
...
- (void)dialog:(NSString *)action
  ...
  [self dialog:action permission:permission andParams:params andDelegate:delegate];
}
...
- (void)dialog:(NSString *)action
	...
	[_fbDialog release];  
	[_permissions release];
	_permissions = [permission retain];
  ...
  if (action == kLogin) {
    ...
    if ([self isSessionValid]) {
	  ...
	  _fbDialog = [[LoginDialog alloc] initWithURLStr:dialogURL params:params delegate:delegate];
  }
	[_fbDialog setNeedWebView:YES];
	[_fbDialog setConnectTitle:@"Connect to Facebook"];
	[_fbDialog setIconImg:[UIImage imageNamed:@"LoginDialog.bundle/images/fbicon.png"]];
	[_fbDialog setNeedWebView:YES];
	[_fbDialog show];
}
 ...
 */
- (BOOL)isSessionValid {
	BOOL isSessionValid = (self.accessToken != nil && self.expirationDate != nil
...
- (void)fbDialogLogin:(NSString *)token expirationDate:(NSDate *)expirationDate {
	[self setAndStoreAccessToken:token expirationDate:(NSDate *)expirationDate];
}
...
- (void)setAndStoreAccessToken:(NSString *)token expirationDate:(NSDate *)expirationDate
{
	...
	[[NSUserDefaults standardUserDefaults] setObject:token forKey:FACEBOOK_STORED_TOKEN_NAME];
	[[NSUserDefaults standardUserDefaults] setObject:expirationDate forKey:FACEBOOK_EXPIRATION_DATE];
...
- (void)login
{
	NSArray *permission = [NSArray arrayWithObjects:@"read_stream",@"publish_stream",nil];
...
{	
	...
	[[NSUserDefaults standardUserDefaults] removeObjectForKey:FACEBOOK_STORED_TOKEN_NAME];
	[[NSUserDefaults standardUserDefaults] removeObjectForKey:FACEBOOK_EXPIRATION_DATE];
	[[NSUserDefaults standardUserDefaults] synchronize];
...
- (void) shareStatus:(NSString *)status
{
	if ([status length] > [self maxStatusLength]) {
		...
		NSError * error = [NSError errorWithDomain:@"FacebookErrDomain" code:toomanyWordsError userInfo:errorData];
		[delegate_ shareItem:self shareFailed:error];
	}else {
		[delegate_ shareItemShareBegin:self];
		NSMutableDictionary * paramDic = [NSMutableDictionary dictionaryWithCapacity:0];
		[paramDic setValue:status forKey:@"message"];
		[self requestWithGraphPath:@"me/feed" 
						 andParams:paramDic 
					 andHttpMethod:@"POST" 
					   andDelegate:self];
	}
...
- (void) shareLink:(NSString *)linkStr
{
	...
	NSMutableDictionary * paramDic = [NSMutableDictionary dictionaryWithCapacity:0];
	[paramDic setValue:linkStr forKey:@"link"];
...
- (void) shareImage:(UIImage *)image withDescription:(NSString *)description
{
	...
	NSMutableDictionary * paramDic = [NSMutableDictionary dictionaryWithCapacity:0];
	[paramDic setValue:description forKey:@"message"];
	[paramDic setValue:image forKey:@"picture"];
...
- (void) shareVideo:(NSData *)videoData withDescription:(NSString *)description
{
	...
	NSMutableDictionary * paramDic = [NSMutableDictionary dictionaryWithCapacity:0];
	[paramDic setValue:videoData forKey:@"video"];
	[paramDic setValue:description forKey:@"description"];
	[paramDic setValue:DEFAULT_VIDEO_NAME forKey:@"title"];
	[paramDic setValue:@"video.upload" forKey:@"method"];
    [self postVideoWithMethodName:@"video.upload" 
						andParams:paramDic 
					andHttpMethod:@"POST" 
					  andDelegate:self];
}
...
{
	//facebook support 420 characters at most
	return 420; 
}
...
#pragma mark -
#pragma mark FBRequestDelegate (For sharing)
...
#pragma mark -
#pragma mark LoginDialogDelegate (For login)
/**
